...
function UserSideBar({ onOpenCreateGroup }) {
    ...
    const [isSearchBarIsClicked, setIsSearchBarIsClicked] = useState(false);
    const [allSearchedElements, setAllSearchedElements] = useState([]);

    const searchBarRef = useRef();
    const searhChatsRef = useRef();
    ...
    useEffect(() => {
        function handleClickOutside(event) {
            if (searchBarRef.current && !searchBarRef.current.contains(event.target)) {
                if (searhChatsRef.current && !searhChatsRef.current.contains(event.target)) {
                    setIsSearchBarIsClicked(false);
                    searchBarRef.current.value = '';
                    setAllSearchedElements([]);
                }
            }
        }
    ...
    const hanldeOnChangeInput = (e) => {
        ...
        setAllSearchedElements([]);
        users.map((user) => {
            const check = user.username.toLowerCase().match(search);
            if (check) setAllSearchedElements(prev => [...prev, user]);
            else if (!check && allSearchedElements.size != 0) {
                allSearchedElements.filter(chat => chat === user)
            }
        })
        groups.map((group) => {
            const check = group.groupName.toLowerCase().match(search);
            if (check) setAllSearchedElements(prev => [...prev, group]);
            else if (!check && allSearchedElements.size != 0) {
                allSearchedElements.map((chat, index) => {
                    chat === group ? allSearchedElements.splice(index, 1) : null;
                })
            }
        })
    }
    ...
    return (
        ...
        <div className='flex h-screen w-full max-w-xs flex-col border-r border-gray-200 bg-white dark:border-slate-700 dark:bg-slate-800'>
            ...
            {/* Filter Tabs */}
            {!isSearchBarIsClicked ? <div className='flex items-center space-x-2 border-b border-gray-200 p-2 dark:border-slate-700'>
                <Button
            ...
            >
                {/* Groups Section */}
                {!isSearchBarIsClicked && selectedChat === 'groups' && groups && groups.length > 0 && (
                    <div className='p-2'>
                        <h3 className='px-2 pt-2 text-xs font-bold tracking-wider text-gray-400 uppercase dark:text-gray-500'>
                            Groups
                        </h3>
                ...
                {/* Users Section */}
                {!isSearchBarIsClicked && selectedChat === 'allchat' && (
                    <div className='p-2'>
                        <h3 className='px-2 pt-2 text-xs font-bold tracking-wider text-gray-400 uppercase dark:text-gray-500'>
                            Users
                        </h3>
                ...
                {isSearchBarIsClicked && (
                    <div className='mt-5'>
                        {/* {console.log()} */}
                        {isSearchBarIsClicked && (
                            <div className='ml-4'>
                                {allSearchedElements.map((chat, index) => {
                                    const isGroupChat = chat && 'groupName' in chat;
                                    const chatName = isGroupChat ? chat.groupName : chat.username;
                                    const chatAvatar = isGroupChat ? chat.avatarURL : chat.avatarURL;
                                    return (
                                        <div key={index}
